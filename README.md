...
수업에서 작성한 wk2/nostyle.html과 new.html을 출발점으로 AI(Codex)를 사용해 HTML 태그를 정리하고, 두 CSS 스타일과 페이지 연결, README 및 퀴즈 초안을 작성했다. 처음 생성된 결과는 원본과 글꼴·배치가 달라 W3Schools의 Stylesheet 1·2와 다시 비교하도록 요청했다. 원본 본문과 CSS에 맞게 수정하고, 이름·학번 및 실제 페이지 링크를 반영했다. 본문 일치 여부, 필수 태그, 링크, 브라우저 화면 검사는 Codex가 수행했다.
...
Codex가 세 실습 페이지의 body 일치, 필수 태그, 내부 링크, HTML 태그 중첩을 검사했다. 브라우저에서 4개 페이지를 열고 390px 화면의 가로 넘침 여부를 확인했다. No Style에는 작성한 CSS가 없고, 스타일 페이지에는 지정한 글꼴과 배치가 적용되는 것을 확인했다. 이후 W3Schools 원본의 Stylesheet 1과 2를 직접 비교하여 글꼴, 제목 크기, 여백, 사이드바 색상 및 배치를 수정했다.
